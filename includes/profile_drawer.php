...
<?php
// Compute base path like navbar so links work from root or /pages
...
<aside class="drawer" id="profileDrawer">
  ...
  <a class="btn danger" href="<?= $__base ?>controllers/logout.php">Logout</a>
...
(function(){
  ...
  const base = (window.APP_ROOT || '').replace(/\/+$/, '/') || '<?= $__base ?>';
  const res = await fetch(base + 'controllers/profile.php', { method:'POST', body: fd });
